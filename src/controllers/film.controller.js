...
const getFilmById = async (req, res) => {
  try {
    if (!req.params.id)
      return res.status(404).send(MoviesEnum.idNotPassed.message());
...
const getFilmByTitle = async (req, res) => {
  try {
    if (!req.params.title)
      return res.status(404).send(MoviesEnum.titleNotPassed.message());
...
const updateFilm = async (req, res) => {
  try {
    if (!req.params.id)
      return res.status(404).send(MoviesEnum.idNotPassed.message());
    if (!req.body.image_url || !req.body.trailer_url)
      return res.status(400).send(MoviesEnum.updateNoValues.message());
